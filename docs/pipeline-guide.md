...
Run stages 1–9 in one shot with `./create_episode.sh <episode-folder>`. `key_footage.py`
and the final render/QA are separate, deliberate steps (see below for why).

## The four scene types that exist today
...
- You can also just describe the change to me in a Claude Code session ("remove the second
  title card", "make the DI Promise clip end 20 frames earlier") and I'll edit the JSON
  directly — this already works today, no special tooling needed.
...
Known limits: hair edges are hard-keyed (not a soft matte), and green spill under strong
lighting isn't fully eliminated by despill — usually fine for talking-head framing, more
